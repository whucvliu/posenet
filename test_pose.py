#-*-coding:UTF-8-*-
# Import the converted model's cla
import numpy as np
import random
...
path = dataFoldPath+'posenet_weight/KingsCollege/181211/'
# Set this path to your dataset directory
directory = dataFoldPath+'KingsCollege/KingsCollege/KingsCollege/'
weightFile=path+'PoseNet_60000.ckpt'
...
def main():
	...
	with tf.Session() as sess:
		...
		for i in range(len(datasource.images)):
			...
			predicted_q = sess.run([p3_q], feed_dict=feed)
			...
			results[i, :] = [theta]
			print 'Iteration:  ', i , '  Error Q (degrees):  ', theta

	median_result = np.median(results,axis=0)
	print 'Median error ',median_result[0], 'degrees.'
